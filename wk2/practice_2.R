...
df = read.table("test_data.csv", sep = ",", header = TRUE, stringsAsFactors = FALSE)
...
# 重新使用str()檢查一次df的結構時...
# Age, Grade, Height, Weight都變成chr型態了?? <- 被kobe資料影響
str(df)
...
df_1 = subset(df, ToeicGrade > 900 & Height > 170, select = c(Name, ToeicGrade, Height))
